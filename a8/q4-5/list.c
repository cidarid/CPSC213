 ...
 */
element_t list_get(struct list *list, int pos) { return list->data[pos]; }
 ...
 */
int list_index(struct list *list, element_t element,
               int (*equal)(element_t, element_t)) {
 ...
 */
int list_len(struct list *list) { return list->len; }

/**
 * Map function f(out,in) onto in_list placing results in out_list (by
 * appending). The lists in_list and out_list must not be the same list. For
 * f(out,in): out is pointer to an element that will be placed in out_list if
 * *out == NULL, f must assign it a value (e.g., by calling malloc) FREE IS NOT
 * CALLED ON ELEMENTS ALLOCATED BY F WHEN THE OUT_LIST IS DESTROYED in  is an
 * element from in_list
 */
...
/**
 * Map function f onto list0 and in_list1 placing results in out_list (by
 * appending). The list out_list must not be one of in_list0 or in_list1. Lists
 * in_list1 and in_list2 can be of different length The length of out_list is
 * equal to the minimum of the lengths of in_list0 and in_list1. For
 * f(out,in0,in1): out is pointer to an element that will be placed in out_list
 *        if *out == NULL, f must assign it a value (e.g., by calling malloc)
 *        FREE IS NOT CALLED ON ELEMENTS ALLOCATED BY F WHEN THE OUT_LIST IS
 * DESTROYED in0  is an element from in_list0 in1  is an element from in_list1
 */
void list_map2(void (*f)(element_t *, element_t, element_t),
               struct list *out_list, struct list *in_list0,
               struct list *in_list1) {
  int length = 0;
 ...
 *        this parameter is included only so that the same f can be used with
 * foldl and map2 it isn't otherwise needed since *out == in0 in1 is an element
 * from in_list
 */
void list_foldl(void (*f)(element_t *, element_t, element_t),
                element_t *out_element_p, struct list *in_list) {
 ...
 * List out_list contains elements of in_list for which f returns true (i.e.,
 * non-zero). NOTE THAT THE OUT_LIST DATA ARRAY ALIASES (I.,E. POINTS TO
 * ELEMENTS OF) THE IN_LIST DATA ARRAY List out_list and in_list must not be the
 * same list. For f(in): in      is an element from in_list returns true (not
 * zero) iff in should be included in out_list and false (zero) otherwise
 */
void list_filter(int (*f)(element_t), struct list *out_list,
                 struct list *in_list) {
  for (int i = 0; i < in_list->len; i++) {
    if (f(in_list->data[i])) {
