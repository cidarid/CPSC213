...
/* Check a new allocation to see if it's legal, and to see if it overlaps with
 * any existing allocations */
static int check_alloc(struct testfunc *tf, void *ptr, int size, void **ptrs,
                       int *sizes, int nptrs) {
  if (ptr == NULL) {
    fprintf(stderr,
            "%s: myheap_malloc returned NULL (maybe it ran out of memory)\n",
            tf->name);
    return -1;
  }
  ...
  if (ptr < (void *)tf->heap || ptr + size > (void *)tf->heap + tf->heapsize) {
    fprintf(stderr,
            "%s: myheap_malloc returned invalid pointer: %p is out of the "
            "bounds of the heap [%p, %p]\n",
  ...
  for (int i = 0; i < nptrs; i++) {
    if (ptr >= ptrs[i] && ptr < ptrs[i] + sizes[i]) {
      fprintf(stderr,
              "%s: myheap_malloc returned an overlapping pointer: %p lies "
              "inside existing allocation [%p, %p]\n",
...
int test_freelist1(struct testfunc *tf) {
  ...
  myheap_free(tf->heap, ptr);
  /* Allocate the same size again. With a free-list, we expect to get the same
   * pointer back. */
  ...
  if (ptr != ptr2) {
    fprintf(
        stderr,
        "%s: myheap_malloc did not reuse free chunk %p (returned %p instead)\n",
        tf->name, ptr, ptr2);
    return -1;
  }
  return 0;
...
int test_freelist2(struct testfunc *tf) {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (ptr != p[0] && ptr != p[1]) {
      fprintf(stderr,
              "%s: myheap_malloc did not reuse either free chunk %p or %p "
              "(returned %p instead)\n",
              tf->name, p[0], p[1], ptr);
      return -1;
    }
...
int test_fixedsize(struct testfunc *tf) {
  /* A harder test which allocates and frees a lot of equal-sized elements many
   * times */
  ...
  for (int i = 0; i < RAND_ITERS; i++) {
    ...
    if (ptrs[idx]) {
      ...
      if (0 != check_alloc(tf, ptr, sizes[idx], ptrs, sizes, RAND_NBUCKETS)) {
        printf(
            "Tried mallocing a value of size %d to payload starting at %p.\n",
            sizes[idx], ptr);
        return -1;
      }
      ptrs[idx] = ptr;
